...
fn get_series_name(attrs: Vec<syn::Attribute>) -> String {
    ...
    for attr in attrs.iter() {
        ...
        if !meta_list.path.is_ident("influx") {
            panic!("unexpected attribute found");
        }
        ...
        for nested_meta in meta_list.nested {
            ...
            let key = name_value.path.get_ident().unwrap();
...
fn record_info(data: syn::Data, suppress_time: bool) -> (Vec<LitInt>, Vec<String>, Vec<Type>) {
...
fn impl_load_fn(
    ...
    let raw_mapping_types =
        vec![syn::parse_str::<syn::Path>("Option<usize>").unwrap(); names.len()];
    let raw_mapping_values = vec![Ident::new("None", Span::call_site()); names.len()];
    let mapping_values = vec![0usize; names.len()];

    let as_types: Vec<Ident> = types
        .iter()
        .map(|ty| {
    ...
    let load_fn_body = quote! {
            ...
            Some(Precision::Seconds))
        {
            Ok(x) => match x
            {
                None => return Err(influx_db_client::Error::DataError("nothing received".to_string())),
                Some(x) => x
            },
            Err(e) => return Err(influx_db_client::Error::DataError(format!("{}", e)))
        };

        // TODO: this is ugly, use and_then?
        let series = match queried.pop()
        {
            None => return Err(influx_db_client::Error::DataError("no results".to_string())),
            Some(x) => match x.series
            {
                None => return Err(influx_db_client::Error::DataError("no series returned".to_string())),
        ...
        {
            return Err(influx_db_client::Error::DataError(
        ...
        for (i, col_name) in series.columns.iter().enumerate()
        {
            match col_name.as_ref()
            {
                ...
                _ =>
                {
                    return Err(influx_db_client::Error::DataError("mapping error".to_string()))
                }
            }
        }
        ...
        {
            Some(x) => x,
            None => return Err(influx_db_client::Error::DataError("unmapped index".to_string()))
        };)*

        let data: Result<Vec<#struct_name>, influx_db_client::Error> =
            series.values.into_iter().map(|val|
        {
            #(let #fields: #types = match &val[mapping.#indices3]
            {
                ...
                {
                    Some(x) => x,
                    None => return Err(influx_db_client::Error::DataError(format!(
                        "cannot convert to {}", #type_names)))
                }
                _ => return Err(influx_db_client::Error::DataError("expected number".to_string()))
            };)*
    ...
    let result = quote! {
        pub fn load(conn: &influx_db_client::Client, query: String)
            -> Result<Vec<#struct_name>, influx_db_client::Error>
        {
...
fn impl_to_point_fn(ast_data: syn::Data, series_name: &String) -> proc_macro2::TokenStream {
    ...
    let value_types: Vec<Path> = types
        .iter()
        .map(|ty| {
...
fn impl_first_fn(struct_name: &Ident, series_name: &String) -> proc_macro2::TokenStream {
    let first_fn_body = quote! {
        ...
        if queried.len() != 1
        {
            return Err(influx_db_client::Error::DataError(format!(
    ...
    let result = quote! {
        pub fn first(conn: &Client) -> Result<#struct_name, influx_db_client::Error>
        {
            #first_fn_body
        }
    };
...
fn impl_last_fn(struct_name: &Ident, series_name: &String) -> proc_macro2::TokenStream {
    ...
    let result = quote! {
        pub fn last(conn: &Client) -> Result<#struct_name, influx_db_client::Error>
        {
            #last_fn_body
        }
    };
...
fn impl_save_fn() -> proc_macro2::TokenStream {
...
fn impl_save_all_fn(struct_name: &Ident) -> proc_macro2::TokenStream {
    let save_all_fn_body = quote! {
        let points: influx_db_client::Points = data.into_iter().map(|x|
        {
            return x.to_point();
        }).collect();
